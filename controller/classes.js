...
const getAllClasses = async (req, res) => {
  ...
  const createClass = async (req, res) => {
    ...
    try {
      const response = await mongodb.getDb().db().collection('classes').insertOne(class_);
       if (response.acknowledged) {
        res.status(201).json({
         ...
         res.status(500).json({ message: 'Failed to create class: Operation not acknowledged by database.' });
      }
    } catch (error) {
      ...
      res.status(500).json({ message: error.message || 'An unexpected error occurred while creating the class.' });
    }
  };
  ...
  const updateClass = async (req, res) => {
   ...
   const response = await mongodb.getDb().db().collection('classes').replaceOne({_id:classId}, class_);
  ...
  const deleteClass = async (req, res) => {
   ...
   if (response.deletedCount > 0){
     ...
     res.status(500).json(response.error || 'Some error occurred while delete the class')
   }
  };
